...
/opt/keycloak/bin/kcadm.sh create users \
    -r "${REALM_NAME}" \
    -s username="${REALM_SERVICE_ACCOUNT_USERNAME}" \
    -s enabled=true

USER_ID=$(/opt/keycloak/bin/kcadm.sh get users \
    -r "${REALM_NAME}" \
    -q username="${REALM_SERVICE_ACCOUNT_USERNAME}" \
    --fields id | grep -o '"id" : "[^"]*' | sed 's/"id" : "//')

/opt/keycloak/bin/kcadm.sh set-password \
    -r "${REALM_NAME}" \
    --userid "${USER_ID}" \
    --new-password "${REALM_SERVICE_ACCOUNT_PASSWORD}"
